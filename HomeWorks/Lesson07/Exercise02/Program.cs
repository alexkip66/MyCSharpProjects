...
Если указанные координаты находятся за пределами границ массива, метод должен вернуть массив с нулевым значением. Если координаты находятся в пределах границ, 
метод должен вернуть массив с двумя значениями: значением числа в указанной позиции, а второй элемент должен быть равен 0, чтобы показать, что операция прошла успешно без ошибок.
...
public class Answer
{
    ...
    {
        // Введите свое решение ниже
        int[] findedNumber = new int[2] { 0, 0 };

        if (rowPosition < matrix.GetLength(0) && columnPosition < matrix.GetLength(1))
            findedNumber[0] = matrix[rowPosition-1, columnPosition-1];
        return findedNumber;
    }
